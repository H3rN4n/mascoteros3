...
angular.module('pets').controller('PetsController', [
	...
	function($scope, $resource, $stateParams, $location, Authentication, Pets, Upload, geolocation, $window, /*Notifications,*/ $http, $timeout, lodash) {
		...
		$scope.setPetAdoption = function (value) {
			$http.put('/api/pets/' + $scope.pet._id + '/adoption', {isAdopt: value}).
				success(function (data, status, headers, config) {
					console.log(data);
					$scope.pet.isAdopt = data.isAdopt;
					//$location.path('pet/' + pet.slug);
				}).
				error(function (data, status, headers, config) {
					// called asynchronously if an error occurs
		...
		$scope.setPetNeutered = function (value) {
			$http.put('/api/pets/' + $scope.pet._id + '/neutered', {neutered: value}).
				success(function (data, status, headers, config) {
					console.log(data);
					$scope.pet.neutered = data.neutered;
					//$location.path('pet/' + pet.slug);
				}).
				error(function (data, status, headers, config) {
					// called asynchronously if an error occurs
		  ...
		  $http.get('/api/users/exists', {
			...
			success(function (data, status, headers, config) {
			  if (data && $scope.pet) {
				...
				$scope.pet.owners.push(data);

				saveOwenerId($scope.pet._id, data._id);
		...
		var saveOwenerId = function(petId, newOwenerId){
			$http.get('/api/pets/' + petId + '/addOwner', {
				params: {
					newOwnerId: newOwenerId
				}
			}).
				success(function (data, status, headers, config) {
					if (data) {
